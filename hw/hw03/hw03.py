...
def pingpong(n):
    ...
    "*** YOUR CODE HERE ***"
# False = down & True = up
    def check_ping(n, index, k, updowncheck):
        while (index < n):
            if (index % 7 == 0 or has_seven(index) == True):
                if (updowncheck == False):
                    return check_ping(n, index + 1, k + 1, True)
                else:
                    return check_ping(n, index + 1, k - 1, False)
            else:
                if (updowncheck == False):
                    return check_ping(n, index + 1, k - 1, False)
                else:
...
def riffle(deck):
    ...
    "*** YOUR CODE HERE ***"
    lengthdeck = len(deck)
    return [deck[i // 2] if i % 2 == 0 else deck[(i * len(deck) // 2 + i // 2 ) % len(deck)] for i in range(lengthdeck)]
