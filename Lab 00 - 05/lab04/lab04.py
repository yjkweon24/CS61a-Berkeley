...
def coords(fn, seq, lower, upper):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [[i, fn(i)] for i in seq if fn(i) >= lower and fn(i) <= 9]
...
def interleave(s0, s1):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(s0, s1, check):
        if (isempty(s1) == True):
            return s0
        elif (isempty(s0) == True):
            return s1
        else:
            if (check == 1):
                return link(first(s0), helper(rest(s0), s1, check * -1))
            else:
                return link(first(s1), helper(s0, rest(s1), check * -1))
    return helper(s0, s1, 1)
