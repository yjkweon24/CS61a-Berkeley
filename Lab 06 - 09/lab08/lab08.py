def make_to_string(front, mid, back, empty_repr):
    ...
    "*** YOUR CODE HERE ***"
    def helper(linking):
        if linking is Link.empty:
            return empty_repr
        else:
            return front + str(linking.first) + mid + helper(linking.rest) + back
    return helper
...
def tree_map(fn, t):
    ...
    helping_loop = [tree_map(fn, i) for i in t.branches]
    return Tree(fn(t.label), helping_loop)
...
def add_trees(t1, t2):
    ...
    making = t1.label + t2.label
    brant1, brant2 = list(t1.branches), list(t2.branches)
    lent1, lent2 = len(brant1), len(brant2)

    if lent1 < lent2:
        brant1 += [None for _ in range(lent1, lent2)]
    elif lent1 > lent2:
        brant2 += [None for _ in range(lent2, lent1)]
    return Tree(making, [add_trees(branch1, branch2) for branch1, branch2 in zip(brant1, brant2)])
# Link
class Link:
    """A linked list.
